"""
在 MCMC（马尔可夫链蒙特卡洛）采样中，我们追求的是非周期性（Aperiodic）。如果一个链有周期性，它就像一个在固定轨道上运行的钟摆，永远无法收敛到一个稳定的分布。
1. 核心数学概念：周期（Period）
状态的周期：从状态 i出发，经过 t步回到状态 。所有可能的步数 t的最大公约数（GCD） 就是该状态的周期。
非周期性：如果一个状态的周期等于 1，则它是非周期的。
非周期马尔可夫链：如果链是不可约的，且所有状态的周期都是 1，则称该链是非周期的。
"""
