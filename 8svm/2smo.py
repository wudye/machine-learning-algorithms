...
class SVM:
    ...
    def smo(self):
        ...
        for k in range(self.max_iter):
            ...
            for i1 in self.outer_circle():  # 外层循环：依据7.4.2.1选择第1个变量（找到a1并更新后继续向后遍历，而不回到第1个）
                i2 = self.inner_circle(i1)  # 内层循环：依据7.4.2.2选择第2个变量（没有处理特殊情况下用启发式规则继续寻找a2）
    ...
    def outer_circle(self):
        """外层循环生成器"""
        """
        边界上的点最敏感：这些支持向量刚好踩在两条隔离带边界上（<span>y_i g(x_i) == 1</span>）。在训练过程中，一旦别的参数更新了，整个边界稍微转动一下，这些刚好踩着线的点立马就会“越线”，从而违反 KKT 条件。
        大部分普通点（<span>\alpha=0</span>）根本不会动：那些离边界十万八千里的安全样本，边界再怎么动，它们也还是安全的（永远满足 KKT），所以去遍历它们纯属浪费时间。
        """
