...
def normalized_string_kernel(s1, s2, length: int, att: float):
    """计算归一化后的字符串核函数的值
    公式：K_norm(s1, s2) = K(s1, s2) / sqrt(K(s1, s1) * K(s2, s2))
    """
    k_s1_s2 = count_kernel_function_for_string(s1, s2, length, att)
    k_s1_s1 = count_kernel_function_for_string(s1, s1, length, att)
    k_s2_s2 = count_kernel_function_for_string(s2, s2, length, att)
    #print(k_s1_s2, k_s1_s1, k_s2_s2)

    # 避免分母为 0 的情况
    if k_s1_s1 == 0 or k_s2_s2 == 0:
        return 0.0

    return k_s1_s2 / math.sqrt(k_s1_s1 * k_s2_s2)


if __name__ == "__main__":
    print(count_kernel_function_for_string("abc", "acd", 2, 0.5))  # 0.03125
    print(normalized_string_kernel("abc", "acd", 2, 0.5))  # 0.25

    # 原始核函数值
    raw_val = count_kernel_function_for_string("fob", "fob", 2, 0.5)
    print("Raw kernel value:", raw_val)  # 0.140625

    # 归一化后的核函数值
    norm_val = normalized_string_kernel("fob", "fob", 2, 0.5)
    print("Normalized kernel value:", norm_val)  # 1.0
